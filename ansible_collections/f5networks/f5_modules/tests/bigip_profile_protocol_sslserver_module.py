...
DOCUMENTATION = r'''
---
module: bigip_profile_protocol_sslserver_module
...
options:
  ...
  sslserver_params:
    description:
      - dictionnaire qui contient les informations du profile sslserver telles qu'écrites dans le fichier.
    type: dict
    required: true
...
# Fonction de création d'un profile sslserver
def fn_creation(sslserver_params, username, password, ip):
    # Initialisation de la commande principale
    cmd = "tmsh\n"
    cmda = ""
    # Construction des blocs de commande à partir des paramètres sslserver
    for key in sslserver_params:
        # On ignore les clés contenant "state"
        if "state" in key:
            pass
        # Commande pour changer de partition
        elif key.strip() == "partition":
            cmd1 = "cd ../" + sslserver_params[key].strip() + "\n"
        # Commande de création du profil sslserver
        elif key.strip() == "name":
            cmd2 = "ltm profile sslserver \ncreate " + sslserver_params['name'].strip()
        # Ajout des autres paramètres à la commande pour la création
        else:
            cmda = cmda + " " + str(key).strip() + " " + str(sslserver_params[key]).strip()
    # commande final
    cmd = cmd + cmd1 + cmd2 + cmda
    return(cmd)

# Fonction pour générer une commande de modification d’un profil sslserver existant
def fn_define_cmd_to_modif(conf_to_modify,sslserver_params):
    # Début de la commande de modification (tmsh + le profile sslserver + le nom du profile qu'on veut modif)
    cmd_mod = "tmsh\nltm profile sslserver\nmodify " + str(sslserver_params['name'])
...
# Fonction pour comparer la configuration existante et celle souhaitée dans sslservers.yml
def fn_compare_conf(sslserver_params, sslserver_infos_from_box):
    to_modify = [] # Liste des paramètres à modifier
    # Parcours des lignes de configuration une à une récupérées depuis le boitier
    for sslserver in range(0,len(sslserver_infos_from_box),1):
        # On ignore toutes les lignes qui possent problème (message inutil, champs inéxistant dans la GUI)
        if "END" in sslserver_infos_from_box[sslserver] or "}" in sslserver_infos_from_box[sslserver] or "app-service" in sslserver_infos_from_box[sslserver].strip() or "defaults-from" in sslserver_infos_from_box[sslserver].strip() or "description" in sslserver_infos_from_box[sslserver].strip() or sslserver_infos_from_box[sslserver] == '' or "Last" in sslserver_infos_from_box[sslserver] or "@" in sslserver_infos_from_box[sslserver] or "ltm" in sslserver_infos_from_box[sslserver]:
          pass
        else:
            # Traitement des lignes contenant "less"
            if "less" in sslserver_infos_from_box[sslserver]:
                # On découpe la ligne en utilisant les espaces multiples comme séparateurs
                new_line = re.split(" +",sslserver_infos_from_box[sslserver])
                # On récupère le nom du champ et sa valeur depuis la ligne
                sslserver_field_from_box = new_line[2].strip()
                sslserver_field_valued_from_box = new_line[3].strip()
                try:
                    # On récupère la valeur attendue depuis sslserver_params via la découpe de la ligne juste au dessus
                    sslserver_field_from_file = sslserver_params[sslserver_field_from_box]
                    # Si les deux valeurs sont identique on pass
                    if str(sslserver_field_valued_from_box) == str(sslserver_field_from_file):
                        pass
                    else:
                        # Si la valeur est différente, on ajoute à la liste des modifications
                        data = sslserver_field_from_box + " " + str(sslserver_params[sslserver_field_from_box])
                        to_modify.append(data)
                # Si le champ n'existe pas dans sslserver_params, on ignore
                except KeyError:
                    pass
            # Traitement des autres lignes
            else:
                # On découpe la ligne en utilisant seulement un espace comme séparateurs
                sslserver_field_from_box = sslserver_infos_from_box[sslserver].strip().split(' ')[0].strip()
                sslserver_field_valued_from_box = sslserver_infos_from_box[sslserver].strip().split(' ')[1].strip()
                try:
                    # On récupère la valeur attendue depuis sslserver_params via la découpe de la ligne juste au dessus
                    sslserver_field_from_file = sslserver_params[sslserver_field_from_box]
                    # Si les deux valeurs sont identique on pass
...
def main():
    ...
    module = AnsibleModule( 
        argument_spec=dict(  
            ...
            sslserver_params = dict(required=True, type='dict')
            ) 
        )
    port = 22
    ...
    sslserver_params = module.params.get('sslserver_params')
    # maintenant que nous avons récupéré les informations passées en argument, nous allons observer le contenu de sslserver_params
    # pour cela, nous lançons une boucle for sur le contenu de la variable et observons l'état du profil sslserver.
    ...
    sslserver_infos_from_box = fn_ssh(cmd, ip, port, username, password)
    not_found = 0
    # Parcours des lignes de configuration une à une récupérées depuis le boîtier
    for i in range(0, len(sslserver_infos_from_box),1):
        # Si le profil n'éxiste pas sur le boîtier, alors on ajout 1
        if "not found" in sslserver_infos_from_box[i]:
            not_found = 1
        # Sinon, on ne fait rien
        else:
            pass
    # Traitement des paramètres du profil sslserver
    for param in sslserver_params:
        cmd = ""
        # Gestion de l'état du profil (présent ou absent)
        if param == "state":
            if sslserver_params[param] != "absent":
                # si le status du profil sslserver dans le fichier est différent de absent alors nous récupérons les informations de ce dernier sur le     # Parcours des lignes de configuration une à une récupérées depuis le boîtier
                if not_found == 1:
                # Si le profil n'existe pas, on effectue les actions suivantes :
                    creation_profile = fn_creation(sslserver_params, username, password, ip)
                    ...
                    module.exit_json(changed=True, message="Le profile sslserver a ete créé", resultat=action_creation)
                else:
                    # Comparaison de la configuration actuelle avec celle souhaitée
                    conf_to_modify = fn_compare_conf(sslserver_params, sslserver_infos_from_box)
                    ...
                    if conf_to_modify != []:
                        ...
                        cmd_to_modify = fn_define_cmd_to_modif(conf_to_modify,sslserver_params)
                        modification_profile = fn_ssh(cmd_to_modify, ip, port, username, password)
                        # Application sur le boîtier via SSH
                        module.exit_json(changed=True, message="Le profile sslserver a ete mis à jour", resultat=modification_profile)
                    else:
                        module.exit_json(changed=False, message="Le profile sslserver sur le boitier est identique à celui du fichier")
            else:
                # vérifier si le profil existe, si oui, lancer la suppression
                if "not found" in sslserver_infos_from_box:
                    ...
                    module.exit_json(changed=False, message="Le profile sslserver a été supprimé", resultat=suppression_profile)
        ...
        # Gestion des des paramètres spécifiques
            ...
            if sslserver_params[param]  != "immediate" or sslserver_params[param] != "indefinite":
                # Si les paramètre on une valeur différente de "immediate" ou de "indefinite"
                sslserver_params[param] = int(sslserver_params[param])
